...
const App: React.FC = () => {
  ...
  // Login is optional - users can browse dashboard without signing in
    ...
    <div className="flex-1 overflow-y-auto p-4 sm:p-8 md:p-12 animate-in fade-in slide-in-from-bottom-6 duration-700 relative">
      <div className="max-w-4xl mx-auto space-y-8 sm:y-12">
        <div className="text-center space-y-3 sm:space-y-4">
          ...
          <p className="text-sm sm:text-base md:text-lg text-slate-500 font-medium max-w-2xl mx-auto px-4">
            {user ? (
              <>Welcome back, <span className="text-blue-600 font-bold">{user.name}</span>! Elevate your hackathon performance with specialized GDG tools.</>
            ) : (
              <>Welcome to <span className="text-blue-600 font-bold">GDG on Campus SRMCEM</span>! Explore our community, events, and AI Assistant.</>
            )}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-8 pb-12 px-2">
          {[
            ...
            <button
              ...
              className={`group relative p-6 sm:p-10 rounded-[2rem] sm:rounded-[3rem] border text-left transition-all duration-500 hover:scale-[1.02] active:scale-[0.98] shadow-lg hover:shadow-xl overflow-hidden ${darkMode ? 'bg-slate-800/40 border-slate-700/50 hover:bg-slate-800' : 'bg-white border-slate-100'
                }`}
            >
      ...
      <button
        ...
        className={`fixed bottom-6 right-6 sm:bottom-8 sm:right-8 w-12 h-12 sm:w-14 sm:h-14 rounded-full flex items-center justify-center shadow-2xl transition-all duration-300 hover:scale-110 active:scale-90 z-50 ${darkMode ? 'bg-slate-800 text-blue-400 border border-slate-700' : 'bg-white text-blue-600 border border-slate-100'
          }`}
        title="About the App"
      >
      ...
      <div className={`flex flex-col h-[100dvh] w-full max-w-4xl shadow-2xl relative overflow-hidden transition-all duration-300 ${darkMode ? 'bg-slate-900 border-x border-slate-800' : 'bg-white'}`}>
          ...
          <div className="relative" ref={menuRef}>
            {user ? (
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl font-bold text-sm transition-all active:scale-95 ${darkMode ? 'bg-blue-600 hover:bg-blue-500 text-white' : 'bg-blue-600 hover:bg-blue-700 text-white'}`}
              >
              ...
              {messages.map((m) => (
                ...
                >
                  <div className="flex-shrink-0 mt-auto mb-1 relative">
                    {m.sender === 'user' ? (
                      <img src={user?.photoURL || `https://ui-avatars.com/api/?name=${user?.name || 'Guest'}`} className="w-8 h-8 sm:w-9 sm:h-9 md:w-11 md:h-11 rounded-full object-cover shadow-xl ring-2 ring-blue-500/10" alt="User" />
                    ) : (
        ...
        {showAuthModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className={`relative w-full max-w-md ${darkMode ? 'bg-slate-900' : 'bg-white'} rounded-3xl shadow-2xl overflow-hidden`}>
              <button
              ...
              <Auth onLogin={(newUser) => { handleLogin(newUser); setShowAuthModal(false); }} />
            </div>
          </div>
        )}
      </div>
